"""Management command to scrape Korn Ferry Tour schedule."""
from django.core.management.base import BaseCommand
from django.db import transaction
from events.models import Tour, Venue, Event
from events.scrapers import KornFerryTourScraper
from events.utils import geocode_venue


class Command(BaseCommand):
    help = 'Scrape and import Korn Ferry Tour schedule'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('Starting Korn Ferry Tour scrape...')

        if not dry_run:
            tour, created = Tour.objects.get_or_create(
                name='Korn Ferry Tour',
                defaults={'website_url': 'https://www.pgatour.com/korn-ferry-tour'}
            )
            if created:
                self.stdout.write(self.style.SUCCESS('Created Korn Ferry Tour'))
        else:
            tour = None

        try:
            with KornFerryTourScraper() as scraper:
