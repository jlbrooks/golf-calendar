"""Management command to scrape LIV Golf schedule."""
from django.core.management.base import BaseCommand
from django.db import transaction
from events.models import Tour, Venue, Event
from events.scrapers import LIVGolfScraper
from events.utils import geocode_venue


class Command(BaseCommand):
    help = 'Scrape and import LIV Golf schedule'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('Starting LIV Golf scrape...')

        if not dry_run:
            tour, created = Tour.objects.get_or_create(
                name='LIV Golf',
                defaults={'website_url': 'https://www.livgolf.com'}
            )
            if created:
                self.stdout.write(self.style.SUCCESS('Created LIV Golf Tour'))
        else:
            tour = None

        try:
            with LIVGolfScraper() as scraper:
